...
def gencontext():
    context = ts.context(ts.SCHEME_TYPE.CKKS, 32768, coeff_mod_bit_sizes=[60,60,60,60])
...
def encrypt(context, np_vector):
    return ts.ckks_vector(context, np_vector)
...
def load_encrypted_vector(file_path, context, batch=None):
    with open(file_path, 'rb') as file:
        ...
        vector = ts.ckks_vector_from(context, serialized_data)
    return vector
...
def search(file_path_folder, img_path,context):
    ...
    for i in range(0, 14):
        ...
        file_path = file_path_folder + str(i) + ".txt"
        encrypted_data = load_encrypted_vector(file_path, context)
        t = time.time()
        en_numerator = encrypted_img.dot(encrypted_data).square_()
    ...
    print("最佳匹配图片为第{}张，检索总计用时{}s，平均单张图片使用余弦距离进行匹配用时{}".format(number, np.sum(time_list), np.mean(time_list)))
...
if __name__ == "__main__":
    ...
    search(file_path_folder, target_image_path, context)  # 检索匹配图像，并计算匹配用时
